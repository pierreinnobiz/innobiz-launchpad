...
const TwoWaysSection: React.FC = () => {
  ...
  const comparisonRows = [
    ...
    { label: t3(language, 'Branding', 'Branding', 'Branding'), wl: t3(language, 'Entièrement personnalisé : diffuseur, accessoires, packaging', 'Fully custom: diffuser, accessories, packaging', 'Totalmente personalizado: difusor, accesorios, packaging'), stock: t3(language, 'Tolia brandé, option fourreau co-brandé', 'Branded Tolia, co-branded sleeve option', 'Tolia con marca, opción de funda co-brandada') },
    { label: t3(language, 'MOQ', 'MOQ', 'MOQ'), wl: t3(language, 'Sur projet (généralement 3 000+)', 'Project-based (typically 3,000+)', 'Por proyecto (típicamente 3.000+)'), stock: t3(language, 'À partir de 500 unités', 'From 500 units', 'Desde 500 unidades') },
    { label: t3(language, 'Personnalisation', 'Customization', 'Personalización'), wl: t3(language, 'Couleur (RAL/Pantone), finitions (mat, soft-touch, métallique), matériaux (bois, résine, verre soufflé), logo, coffret complet', 'Color (RAL/Pantone), finishes (matte, soft-touch, metallic), materials (wood, resin, blown glass), logo, full box', 'Color (RAL/Pantone), acabados (mate, soft-touch, metálico), materiales (madera, resina, vidrio soplado), logo, caja completa'), stock: t3(language, 'Fourreau extérieur co-brandé, packaging intérieur neutre', 'Co-branded outer sleeve, neutral inner packaging', 'Funda exterior co-brandada, packaging interior neutro') },
  ...
  const cards = [
    ...
    },
    {
      ...
      bullets: [
        t3(language, 'Stock prêt à expédier, minimum 500 unités, depuis notre entrepôt français', 'Ready-to-ship inventory, minimum 500 units, from our French warehouse', 'Inventario listo para enviar, mínimo 500 unidades, desde nuestro almacén francés'),
